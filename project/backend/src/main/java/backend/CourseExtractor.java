...
public class CourseExtractor {
	...
	public CourseExtractor(BufferedReader s){
		this.s = s;
	}
	...
	public Course run() throws Exception{
		...
		String coursecode = line1.substring(m.start(), m.end());
		
		// I am considering the course name to be just the second line
		String coursename = file.split("\n")[1];
		...
		k = Pattern.compile("20\\d{2}-(0[1-9]|1[0-2])-((0[1-9])|(1\\d)|(2\\d)|(30)|(31))[\\t \\n]+([0-1]\\d|2[0-4]):((0\\d)|(1\\d)|(2\\d)|(3\\d)|(4\\d)|(5\\d)):((0\\d)|(1\\d)|(2\\d)|(3\\d)|(4\\d)|(5\\d))").matcher(markable);
		k.find();
